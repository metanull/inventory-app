...
describe('CountryTranslationImporter - Data Transformation', () => {
  ...
  it('should transform each sample correctly', async () => {
    const samples = helper.loadSamples<CountryTranslationSample>('country_translation', { limit: 20 });
    ...
    const calls = helper.getApiCalls(mockContext.apiClient, 'countryTranslation.countryTranslationStore');
    ...
    samples.forEach((sample, i) => {
      ...
      expect(apiData.backward_compatibility).toBe(`mwnf3:countrynames:${sample.country}:${sample.lang}`);
    });
  });

  it('should map non-standard country codes correctly', () => {
    const samples = helper.loadSamples<CountryTranslationSample>('country_translation', { limit: 50 });
