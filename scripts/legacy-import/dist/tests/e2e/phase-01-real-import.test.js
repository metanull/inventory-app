...
const LegacyDatabase_js_1 = require("../../src/database/LegacyDatabase.js");
const InventoryApiClient_js_1 = require("../../src/api/InventoryApiClient.js");
const PartnerImporter_js_1 = require("../../src/importers/phase-01/PartnerImporter.js");
const ProjectImporter_js_1 = require("../../src/importers/phase-01/ProjectImporter.js");
 ...
 */
vitest_1.describe.skip('E2E: Phase 1 Real Import', () => {
    ...
    (0, vitest_1.it)('should import projects from legacy database', async () => {
        const context = {
            legacyDb,
            apiClient,
            tracker,
        ...
        const importer = new ProjectImporter_js_1.ProjectImporter(context);
        const result = await importer.import();
        // Verify import succeeded
        (0, vitest_1.expect)(result.success).toBe(true);
        (0, vitest_1.expect)(result.imported).toBeGreaterThan(0);
        (0, vitest_1.expect)(result.imported).toBeLessThanOrEqual(2);
        // Verify contexts were created via API
        const contextsResponse = await apiClient.context.contextIndex();
        (0, vitest_1.expect)(contextsResponse.data.data.length).toBeGreaterThan(0);
        // Verify collections were created
        const collectionsResponse = await apiClient.collection.collectionIndex();
        (0, vitest_1.expect)(collectionsResponse.data.data.length).toBeGreaterThan(0);
    ...
    (0, vitest_1.it)('should import partners (museums + institutions) from legacy database', async () => {
        const context = {
            legacyDb,
            apiClient,
            tracker,
        ...
        const importer = new PartnerImporter_js_1.PartnerImporter(context);
        const result = await importer.import();
        // Verify import succeeded
        (0, vitest_1.expect)(result.success).toBe(true);
        (0, vitest_1.expect)(result.imported).toBeGreaterThan(0);
        (0, vitest_1.expect)(result.imported).toBeLessThanOrEqual(4); // 2 museums + 2 institutions max
        // Verify partners were created via API
        const partnersResponse = await apiClient.partner.partnerIndex();
        (0, vitest_1.expect)(partnersResponse.data.data.length).toBeGreaterThan(0);
        ...
        (0, vitest_1.expect)(hasMuseum || hasInstitution).toBe(true);
    }, 30000);
    (0, vitest_1.it)('should handle deduplication on repeated imports', async () => {
        const context = {
            legacyDb,
            apiClient,
            tracker,
        ...
        // First import
        const importer1 = new PartnerImporter_js_1.PartnerImporter(context);
        const result1 = await importer1.import();
        // Second import with same tracker (should skip)
        const importer2 = new PartnerImporter_js_1.PartnerImporter(context);
        const result2 = await importer2.import();
        // Second import should skip everything
        (0, vitest_1.expect)(result2.skipped).toBeGreaterThanOrEqual(result1.imported);
        (0, vitest_1.expect)(result2.imported).toBe(0);
        console.log('\n✓ Deduplication works:');
 ...
 */
//# sourceMappingURL=phase-01-real-import.test.js.map