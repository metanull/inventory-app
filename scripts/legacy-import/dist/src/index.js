...
const LoginHelper_js_1 = require("./utils/LoginHelper.js");
const InventoryApiClient_js_1 = require("./api/InventoryApiClient.js");
const LegacyDatabase_js_1 = require("./database/LegacyDatabase.js");
...
program
    ...
    try {
        ...
        await (0, LoginHelper_js_1.quickLogin)(baseUrl);
...
program
    ...
    try {
        ...
        const apiClient = (0, InventoryApiClient_js_1.createApiClient)();
    ...
    try {
        ...
        const legacyDb = (0, LegacyDatabase_js_1.createLegacyDatabase)();
...
program
    ...
    try {
        ...
        let languageResult = { imported: 0, skipped: 0, errors: [] };
        let countryResult = { imported: 0, skipped: 0, errors: [] };
        let defaultContextResult = { imported: 0, skipped: 0, errors: [] };
        let projectResult = { imported: 0, skipped: 0, errors: [] };
        let partnerResult = { imported: 0, skipped: 0, errors: [] };
        let objectResult = { imported: 0, skipped: 0, errors: [] };
        let monumentResult = { imported: 0, skipped: 0, errors: [] };
        const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
        // Setup log file
        const { writeFileSync, mkdirSync, appendFileSync } = await Promise.resolve().then(() => __importStar(require('fs')));
        ...
        const { BackwardCompatibilityTracker } = await Promise.resolve().then(() => __importStar(require('./utils/BackwardCompatibilityTracker.js')));
        ...
        writeLog('-'.repeat(60));
        const { LanguageImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-00/LanguageImporter.js')));
        const { CountryImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-00/CountryImporter.js')));
        const { DefaultContextImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-00/DefaultContextImporter.js')));
        // Languages
        ...
        if (phase === 1 || phase === 'all') {
            ...
            writeLog('-'.repeat(60));
            const { PartnerImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-01/PartnerImporter.js')));
            const { ProjectImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-01/ProjectImporter.js')));
            // Import projects first
            console.log('Projects: ');
            writeLog('\nProjects Import:');
            const projectImporter = new ProjectImporter(importContext);
            projectResult = await projectImporter.import();
            console.log(` ${projectResult.imported} imported, ${projectResult.skipped} skipped, ${projectResult.errors.length} errors\n`);
            ...
            console.log(` ${partnerResult.imported} imported, ${partnerResult.skipped} skipped, ${partnerResult.errors.length} errors\n`);
            ...
            const { ObjectImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-01/ObjectImporter.js')));
            const { MonumentImporter } = await Promise.resolve().then(() => __importStar(require('./importers/phase-01/MonumentImporter.js')));
            // Import objects
            console.log('Objects: ');
            writeLog('\nObjects Import:');
            const objectImporter = new ObjectImporter(importContext);
            objectResult = await objectImporter.import();
            console.log(` ${objectResult.imported} imported, ${objectResult.skipped} skipped, ${objectResult.errors.length} errors\n`);
            ...
            console.log(` ${monumentResult.imported} imported, ${monumentResult.skipped} skipped, ${monumentResult.errors.length} errors\n`);
        ...
        const totalImported = (languageResult?.imported || 0) +
        ...
        const totalSkipped = (languageResult?.skipped || 0) +
        ...
        const totalErrors = (languageResult?.errors.length || 0) +
        ...
        console.log(`  Status: ${totalErrors === 0 ? '\x1b[32m✓ SUCCESS\x1b[0m' : '\x1b[31m✗ FAILED\x1b[0m'}`);
        console.log('='.repeat(60) + '\n');
