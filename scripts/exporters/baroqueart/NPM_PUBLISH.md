...
`@metanull/baroqueart-data` on GitHub Packages. What the package *contains*
...
- **name** — `@metanull/baroqueart-data` (hardcoded in the exporter CLI)
...
```javascript
import manifest from '@metanull/baroqueart-data/manifest.json' assert { type: 'json' }
import items from '@metanull/baroqueart-data/items.json' assert { type: 'json' }

// Lazy-load translations for a language
const { default: t } = await import(`@metanull/baroqueart-data/translations/items.${lang}.json`)
```
