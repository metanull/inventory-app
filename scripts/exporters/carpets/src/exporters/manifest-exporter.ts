 ...
 *
 * `languages` lists the gallery's own UI languages (`thg_gallery_lang` — carpets
 * ships ar/en/es/fr), not every language in the database. It is metadata only:
 * following the baroqueart decision, a viewer decides what it can offer from the
 * translation files that are actually present, since an individual record may
 * carry more languages than the gallery shipped in.
 */
