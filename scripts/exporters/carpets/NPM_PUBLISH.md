...
`@metanull/carpets-data` on GitHub Packages. What the package *contains* is
...
- **name** — `@metanull/carpets-data` (hardcoded in the exporter CLI)
...
```bash
npm install @metanull/carpets-data
```

```javascript
import manifest from '@metanull/carpets-data/manifest.json' assert { type: 'json' }
import gallery from '@metanull/carpets-data/gallery.json' assert { type: 'json' }
import items from '@metanull/carpets-data/items.json' assert { type: 'json' }

// Lazy-load translations for a language
const { default: t } = await import(`@metanull/carpets-data/translations/items.${lang}.json`)
```
