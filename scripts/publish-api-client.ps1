...
if (-not (Test-Path -Path $ClientPath)) {
    ...
    Write-Information "Run generate-api-client.ps1 first to generate the client." -InformationAction Continue
    exit 1
}
...
Write-Information "Publishing API client..." -InformationAction Continue
Write-Information "Package: $PackageName" -InformationAction Continue
Write-Information "Version: $PackageVersion" -InformationAction Continue
Write-Information "Registry: $Registry" -InformationAction Continue
...
if (-not $whoami) {
    Write-Information "You are not logged in to $Registry. Running 'npm login'..." -InformationAction Continue
    if ($Credential) {
...
if ($PackageVersion -match '-') {
    $PublishArgs += @('--tag', 'dev')
    Write-Information "Detected prerelease version, adding --tag dev to npm publish." -InformationAction Continue
}
if ($DryRun) {
    $PublishArgs += "--dry-run"
    Write-Information "Performing dry run..." -InformationAction Continue
} else {
    Write-Information "Publishing to npm..." -InformationAction Continue
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        if ($DryRun) {
            Write-Information "✔ Dry run completed successfully!" -InformationAction Continue
            Write-Information "Package would be published as: $PackageName@$PackageVersion" -InformationAction Continue
        } else {
            Write-Information "✔ Package published successfully!" -InformationAction Continue
            Write-Information "Published: $PackageName@$PackageVersion" -InformationAction Continue
            Write-Information "Install with: npm install $PackageName" -InformationAction Continue
        }
    } else {
