        ...
        return $table
            ...
            ->columns([
                ImageColumn::make('preview')
                    ->label('Preview')
                    ->getStateUsing(fn ($record) => route('filament.admin.collection-image.view', [
                        'collection' => $record->collection_id,
                        'collectionImage' => $record->id,
                    ]))
                    ->height(64)
                    ->url(fn ($record) => route('filament.admin.collection-image.view', [
                        'collection' => $record->collection_id,
                        'collectionImage' => $record->id,
                    ]))
                    ->openUrlInNewTab()
            ...
            ->headerActions([
                Action::make('attach')
                    ...
                    ->action(function (array $data): void {
                        ...
                        CollectionImage::attachFromAvailableImage(
                            $availableImage,
                            $collection->id,
                            $data['alt_text'] ?? null
                        );
            ...
            ->actions([
                Action::make('view_image')
                    ...
                    ->icon('heroicon-o-eye')
                    ->color('gray')
                    ->url(fn (CollectionImage $record) => route('filament.admin.collection-image.view', [
                        'collection' => $record->collection_id,
                        'collectionImage' => $record->id,
                    ]))
                    ->openUrlInNewTab(),
                Action::make('download')
                    ...
                    ->color('gray')
                    ->url(fn (CollectionImage $record) => route('filament.admin.collection-image.download', [
                        'collection' => $record->collection_id,
                        'collectionImage' => $record->id,
                    ])),
                EditAction::make()
                ...
                Action::make('detach')
                    ...
                    ->action(function (CollectionImage $record): void {
                ...
                DeleteAction::make()
                    ...
                    ->before(function (CollectionImage $record): void {
