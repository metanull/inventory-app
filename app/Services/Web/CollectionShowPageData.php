...
class CollectionShowPageData
{
    ...
    public function build(Collection $collection): array
    {
        $collection->load([
            ...
            'language',
            'parent',
            'children' => fn (Relation $query) => $query->orderBy('display_order'),
            'translations.context',
            'translations.language',
            'attachedItems.itemImages' => fn (Relation $query) => $query->orderBy('display_order'),
            'collectionImages' => fn (Relation $query) => $query->orderBy('display_order'),
        ]);
