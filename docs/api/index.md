...
Interactive REST API documentation for managing museum inventory data.

## Quick Access

- [Interactive API Explorer (Swagger UI)]({{ '/swagger-ui.html' | relative_url }}) - Test endpoints directly
- [OpenAPI Specification]({{ '/api.json' | relative_url }}) - Download the full specification
- [TypeScript Client]({{ '/api-client/' | relative_url }}) - Auto-generated TypeScript client library
...
The Inventory Management API provides RESTful endpoints for managing museum inventory data with support for:

- **Multi-language translations** - Full internationalization support
- **Image management** - Upload, process, and attach images to items and collections
- **Hierarchical collections** - Organize items into collections, galleries, and exhibitions
- **Partner management** - Track institutional partners and contributors
- **Authentication** - Token-based API access via Laravel Sanctum

### System Endpoints

- `GET /api/info` - Application information and health status
...
1. **Upload** → `POST /api/image-upload` - Upload and queue for processing
2. **Processing** → Background processing resizes and optimizes images
3. **Attachment** → Attach processed images to items, collections, or partners

For detailed image workflow documentation, see the [ImageUpload]({{ '/models/' | relative_url }}) and [ItemImage]({{ '/models/' | relative_url }}) model pages.
