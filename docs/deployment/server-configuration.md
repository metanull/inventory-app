...
```ini
; /etc/php/8.5/fpm/pool.d/inventory-app.conf
[inventory-app]
user = www-data
group = www-data
listen = /var/run/php/php8.5-fpm-inventory-app.sock
...
; Logging
php_admin_value[error_log] = /var/log/php8.5-fpm-inventory-app.log
...
1. **502 Bad Gateway (Nginx)**
   - Check PHP-FPM status: `systemctl status php8.5-fpm`
