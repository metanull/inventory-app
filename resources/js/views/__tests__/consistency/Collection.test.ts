...
describe('Collection Consistency Tests', () => {
  const collectionListPath = path.resolve(__dirname, '../../Collections.vue')
  const collectionDetailPath = path.resolve(__dirname, '../../CollectionDetail.vue')

  describe('Collections.vue - List View Compliance', () => {
    ...
    it('should use format components instead of raw data in table cells', () => {
      ...
      expect(content).toContain('<DateDisplay :date="collection.created_at"')
    })
  ...
  describe('CollectionDetail.vue - Detail View Compliance', () => {
    ...
    it('should use format components for data display instead of raw values', () => {
      ...
      // CollectionDetail.vue no longer uses DateDisplay - timestamps are handled by system properties component
  ...
  describe('Navigation and Menu Integration', () => {
    ...
    it('should use named routes consistently for navigation', () => {
      ...
      expect(content).toContain("router.push({ name: 'collections' })")

      // Should NOT use string paths for main navigation
      expect(content).not.toContain("router.push('/collections')")
      expect(content).not.toContain('router.push("/collections")')
    })
