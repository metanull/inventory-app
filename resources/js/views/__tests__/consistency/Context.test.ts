...
describe('Context Consistency Tests', () => {
  const contextListPath = path.resolve(__dirname, '../../Contexts.vue')
  const contextDetailPath = path.resolve(__dirname, '../../ContextDetail.vue')

  describe('Contexts.vue - List View Compliance', () => {
    it('should use ErrorDisplay store for error handling', () => {
      const content = fs.readFileSync(contextListPath, 'utf-8')
    ...
    it('should use format components instead of raw data in table cells', () => {
      ...
      expect(content).toContain('<DateDisplay :date="context.created_at"')
    })
    ...
    it('should use centralized color system consistently', () => {
      ...
      // Should default to 'green' for contexts
  ...
  describe('ContextDetail.vue - Detail View Compliance', () => {
    it('should use CancelChangesConfirmation store for unsaved changes protection', () => {
      const content = fs.readFileSync(contextDetailPath, 'utf-8')
  ...
  describe('Navigation and Menu Integration', () => {
    ...
    it('should use named routes consistently for navigation', () => {
      ...
      expect(content).toContain("router.push({ name: 'contexts' })")

      // Should NOT use string paths for main navigation
      expect(content).not.toContain("router.push('/contexts')")
      expect(content).not.toContain('router.push("/contexts")')
    })
