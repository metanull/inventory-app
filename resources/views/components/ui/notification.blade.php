...
<div 
    class="mb-4 rounded-md {{ $colors['bg'] }} p-4 border {{ $colors['border'] }}" 
    ...
    @if($autoClose)
    x-init="setTimeout(() => { show = false }, {{ $autoClose }})"
    @endif
>
    <div class="flex items-center justify-between">
        <div class="flex items-center">
            ...
            @endif
            <p class="ml-3 text-sm font-medium {{ $colors['text'] }}">{{ $message }}</p>
        </div>
        @if($dismissible)
            <button type="button" class="ml-3 {{ $colors['button'] }}" @click="show = false">
