...
<div class="mt-8">
    <x-layout.section title="Spellings" icon="document-text">
        <x-slot name="action">
            <x-ui.button 
                href="{{ route('glossaries.spellings.create', $model) }}" 
                variant="primary" 
                icon="plus">
                Add Spelling
            </x-ui.button>
        ...
        @else
            <div class="space-y-4">
                @php
                    $spellingsByLanguage = $model->spellings->groupBy('language_id');
                @endphp
                                ...
                                @foreach($spellings as $spelling)
                                    <div class="flex items-center justify-between py-2 px-3 bg-gray-50 rounded">
                                        ...
                                        <div class="flex gap-2">
                                            <!-- View -->
                                            <x-ui.button 
                                                href="{{ route('glossaries.spellings.show', [$model, $spelling]) }}" 
                                                variant="edit"
                                                size="sm"
                                                icon="eye">
                                                View
                                            ...
                                            <!-- Edit -->
                                            <x-ui.button 
                                                href="{{ route('glossaries.spellings.edit', [$model, $spelling]) }}" 
                                                variant="warning"
                                                size="sm"
                                                icon="pencil">
                                                Edit
                                            ...
                                            <!-- Delete -->
                                            <x-ui.confirm-button 
                                                action="{{ route('glossaries.spellings.destroy', [$model, $spelling]) }}"
