    ...
    <div class="space-y-4">
        ...
        <div class="flex flex-wrap gap-3">
            <div class="flex items-center gap-2">
                <label class="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">Type:</label>
                <select
            ...
            <div class="flex items-center gap-2">
                <label class="text-sm text-gray-600 dark:text-gray-400 whitespace-nowrap">Project:</label>
                <select
                    wire:model.live="filterProject"
                    class="fi-input rounded-lg border border-gray-300 bg-white px-2 py-1.5 text-sm shadow-sm dark:border-white/10 dark:bg-gray-800 dark:text-white"
                >
                    <option value="all">All</option>
                    <option value="with">With project</option>
                    <option value="without">Without project</option>
                </select>
            </div>
            <div class="flex items-center gap-2">
            ...
            @if ($filterType !== 'all' || $filterProject !== 'all' || $filterCountry !== 'all')
                <button
                    wire:click="$set('filterType', 'all'); $set('filterProject', 'all'); $set('filterCountry', 'all')"
        ...
        @elseif ($rootCount > $roots->count())
            <div class="px-1 text-sm text-gray-500 dark:text-gray-400">
                Showing {{ $roots->count() }} of {{ $rootCount }} root items &mdash; page {{ $page }} of {{ $totalPages }}.
            </div>
        @else
            <div class="px-1 text-sm text-gray-500 dark:text-gray-400">
                {{ $rootCount }} {{ Str::plural('root item', $rootCount) }}
            </div>
        @endif
