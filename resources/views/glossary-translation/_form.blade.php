...
<div class="p-6 space-y-6">
        ...
        <select name="language_id" class="block w-full px-3 py-2 rounded-md shadow-sm sm:text-sm border border-gray-300 focus:border-indigo-500 focus:ring-indigo-500" required>
            ...
            @foreach($languages as $language)
                @php
                    ...
                    $isSelected = old('language_id', $translation->language_id ?? '') == $language->id;
                @endphp
    ...
    <x-form.field label="Definition" name="definition" variant="gray" required>
        ...
        >{{ old('definition', $translation->definition ?? '') }}</textarea>
    </x-form.field>
</div>

<x-form.actions 
    entity="translation" 
    :cancel-route="$translation ? route('glossaries.translations.show', [$glossary, $translation]) : route('glossaries.translations.index', $glossary)"
/>
