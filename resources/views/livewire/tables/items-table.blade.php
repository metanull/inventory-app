<div class="space-y-4">
    ...
    @if($hierarchyMode)
        <div class="flex items-center gap-2 flex-wrap">
            ...
            @else
                <span class="text-sm font-medium text-gray-700">Root items</span>
            @endif
    ...
    @else
        <div class="flex items-center justify-between">
            <span class="text-sm text-gray-500">Showing all items</span>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @forelse($items as $item)
                    @php($hasChildren = $hierarchyMode && $item->children_count > 0)
                    <tr class="{{ $hasChildren ? 'hover:bg-blue-50 cursor-pointer' : 'hover:bg-gray-50 cursor-pointer' }}"
                        wire:key="item-{{ $item->id }}"
                        @if($hasChildren)
                            wire:click="navigateToParent('{{ $item->id }}')"
                        @else
                            onclick="window.location='{{ route('items.show', $item) }}'"
                        @endif
                    >
                        ...
                        @if($hierarchyMode)
                            <td class="hidden sm:table-cell px-4 py-3 text-sm text-gray-500">
                                @if($item->children_count > 0)
                                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800">{{ $item->children_count }}</span>
                                @else
