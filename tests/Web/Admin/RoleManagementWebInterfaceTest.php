...
class RoleManagementWebInterfaceTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        // Create user with MANAGE_ROLES permission for all tests
        $this->user = User::factory()->create(['email_verified_at' => now()]);
        $this->user->givePermissionTo(PermissionEnum::MANAGE_ROLES);
    ...
    public function test_user_can_add_permission_to_role(): void
    {
        ...
        $response = $this->put(route('admin.roles.updatePermissions', $role), [
            'action' => 'add',
    ...
    public function test_user_can_sync_all_permissions_at_once(): void
    {
        ...
        $updatePermission = Permission::where('name', PermissionEnum::UPDATE_DATA->value)->first();

        // Give role some initial permissions
        $role->givePermissionTo([$viewPermission, $createPermission]);

        $response = $this->put(route('admin.roles.updatePermissions', $role), [
            'action' => 'sync',
            'permissions' => [$updatePermission->id], // Only update permission
        ]);
        ...
        $role->refresh();
        $this->assertFalse($role->hasPermissionTo(PermissionEnum::VIEW_DATA->value));
        $this->assertFalse($role->hasPermissionTo(PermissionEnum::CREATE_DATA->value));
