<template>
  <div class="mt-8 space-y-6">
    ...
    <div class="flex items-center justify-between">
      <h3 class="text-lg font-medium leading-6 text-gray-900">Collection Images</h3>
      <button
        type="button"
        :class="[
    ...
    >
      ...
      <p class="mt-1 text-sm text-gray-500">
        Get started by attaching an image to this collection.
      </p>
      <div class="mt-6">
        <button
          type="button"
...
<script setup lang="ts">
  ...
  const images = computed(() => collectionImageStore.collectionImages)
  const loading = computed(() => collectionImageStore.loading)
  const availableImages = computed(() => availableImageStore.availableImages || [])

  // Image URL caches
  const collectionImageUrls = ref<Record<string, string>>({})
  const loadingCollectionImageUrls = ref<Record<string, boolean>>({})
  ...
  // Get collection image URL via API (fetches as blob to support authentication)
  const getCollectionImageUrl = (image: CollectionImageResource): string => {
    // Return cached URL if available
    if (collectionImageUrls.value[image.id]) {
      return collectionImageUrls.value[image.id]!
    }

    // If not already loading, start loading
    if (!loadingCollectionImageUrls.value[image.id]) {
      loadingCollectionImageUrls.value[image.id] = true
      collectionImageStore
        .getImageUrl(image)
        .then(url => {
          collectionImageUrls.value[image.id] = url
          loadingCollectionImageUrls.value[image.id] = false
        })
        .catch(error => {
          console.error('Failed to load collection image URL:', error)
          loadingCollectionImageUrls.value[image.id] = false
          // Use fallback placeholder
          collectionImageUrls.value[image.id] =
            'data:image/svg+xml;base64,'
        })
    }
  ...
  // Load images when component mounts or collectionId changes
  const loadImages = async () => {
    if (props.collectionId) {
      try {
        await collectionImageStore.fetchCollectionImages(props.collectionId)
      } catch (error) {
        console.error('[CollectionImageManager] Failed to load images:', error)
  ...
  const handleAttachImage = async (availableImageId: string) => {
    try {
      ...
      await collectionImageStore.attachImageToCollection(props.collectionId, availableImageId)
      ...
      await Promise.all([
        loadImages(), // Reload collection images
  ...
  const handleMoveUp = async (imageId: string) => {
    try {
      await collectionImageStore.moveImageUp(imageId)
  ...
  const handleMoveDown = async (imageId: string) => {
    try {
      await collectionImageStore.moveImageDown(imageId)
  ...
  const handleUpdateAltText = async (imageId: string, altText: string | null) => {
    try {
      await collectionImageStore.updateCollectionImage(imageId, { alt_text: altText })
    } catch {
  ...
  const handleDetach = async (image: CollectionImageResource) => {
    ...
    if (result === 'delete') {
      try {
        ...
        await collectionImageStore.detachImageFromCollection(image.id)
  ...
  const handleDelete = async (image: CollectionImageResource) => {
    ...
    if (result === 'delete') {
      try {
        ...
        await collectionImageStore.deleteCollectionImage(image.id)
