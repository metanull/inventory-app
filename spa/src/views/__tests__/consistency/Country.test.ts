...
describe('Country Consistency Tests', () => {
  const countryListPath = path.resolve(__dirname, '../../Countries.vue')
  const countryDetailPath = path.resolve(__dirname, '../../CountryDetail.vue')

  describe('Countries.vue - List View Compliance', () => {
    ...
    it('should use format components instead of raw data in table cells', () => {
      ...
      expect(content).toContain('<DateDisplay :date="country.created_at"')
    })
  ...
  describe('CountryDetail.vue - Detail View Compliance', () => {
    ...
    it('should use format components for data display instead of raw values', () => {
      ...
      // CountryDetail.vue no longer uses DateDisplay - timestamps are handled by system properties component
  ...
  describe('Navigation and Menu Integration', () => {
    ...
    it('should use named routes consistently for navigation', () => {
      ...
      expect(content).toContain("router.push({ name: 'countries' })")

      // Should NOT use string paths for main navigation
      expect(content).not.toContain("router.push('/countries')")
      expect(content).not.toContain('router.push("/countries")')
    })
