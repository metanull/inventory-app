...
describe('Partner Consistency Tests', () => {
  const partnerListPath = path.resolve(__dirname, '../../Partners.vue')
  const partnerDetailPath = path.resolve(__dirname, '../../PartnerDetail.vue')

  describe('Partners.vue - List View Compliance', () => {
    it('should use ErrorDisplay store for error handling', () => {
      const content = fs.readFileSync(partnerListPath, 'utf-8')
    ...
    it('should use format components instead of raw data in table cells', () => {
      ...
      expect(content).toContain('<DateDisplay :date="partner.created_at"')
    })
    ...
    it('should use centralized color system consistently', () => {
      ...
      // Should default to 'yellow' for partners
  ...
  describe('PartnerDetail.vue - Detail View Compliance', () => {
    it('should use CancelChangesConfirmation store for unsaved changes protection', () => {
      const content = fs.readFileSync(partnerDetailPath, 'utf-8')
    ...
    it('should use format components for data display instead of raw values', () => {
      ...
      // PartnerDetail.vue no longer uses DateDisplay - timestamps are handled by system properties component
  ...
  describe('Navigation and Menu Integration', () => {
    ...
    it('should use named routes consistently for navigation', () => {
      ...
      expect(content).toContain("router.push({ name: 'partners' })")

      // Should NOT use string paths for main navigation
      expect(content).not.toContain("router.push('/partners')")
      expect(content).not.toContain('router.push("/partners")')
    })
