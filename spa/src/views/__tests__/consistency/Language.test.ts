...
describe('Language Consistency Tests', () => {
  const languageListPath = path.resolve(__dirname, '../../Languages.vue')
  const languageDetailPath = path.resolve(__dirname, '../../LanguageDetail.vue')

  describe('Languages.vue - List View Compliance', () => {
    it('should use ErrorDisplay store for error handling', () => {
      const content = fs.readFileSync(languageListPath, 'utf-8')
    ...
    it('should use format components instead of raw data in table cells', () => {
      ...
      expect(content).toContain('<DateDisplay :date="language.created_at"')
    })
    ...
    it('should use centralized color system consistently', () => {
      ...
      // Should default to 'purple' for languages
  ...
  describe('LanguageDetail.vue - Detail View Compliance', () => {
    it('should use CancelChangesConfirmation store for unsaved changes protection', () => {
      const content = fs.readFileSync(languageDetailPath, 'utf-8')
    ...
    it('should use format components for data display instead of raw values', () => {
      ...
      // LanguageDetail.vue no longer uses DateDisplay - timestamps are handled by system properties component
  ...
  describe('Navigation and Menu Integration', () => {
    ...
    it('should use named routes consistently for navigation', () => {
      ...
      expect(content).toContain("router.push({ name: 'languages' })")

      // Should NOT use string paths for main navigation
      expect(content).not.toContain("router.push('/languages')")
      expect(content).not.toContain('router.push("/languages")')
    })
